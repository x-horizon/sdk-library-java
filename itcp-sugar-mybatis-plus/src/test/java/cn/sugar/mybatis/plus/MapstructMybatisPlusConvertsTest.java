package cn.sugar.mybatis.plus;


import cn.sugar.convert.mapstruct.exception.MapstructConvertMethodNotFoundException;
import cn.sugar.mybatis.plus.bean.domain.StudentDO;
import cn.sugar.mybatis.plus.bean.vo.StudentVO;
import cn.sugar.mybatis.plus.utils.Converts;
...
public class MapstructMybatisPlusConvertsTest {
    ...
    public void testMybatisPlusMapstructBeanConverter() {
        // 转换成功
        Assert.assertNotNull(Converts.withMybatisPlusMapstruct().toPageBean(DEFAULT_PAGE, StudentVO.class).getRecords().get(0));
        // 转换带默认值，源对象为 null 的情况下，将默认值包装为 PageResult<默认值> 并返回
        Assert.assertNotNull(Converts.withMybatisPlusMapstruct().toPageBean(null, StudentVO.class, DEFAULT_STUDENT_VO).getRecords().get(0));
        ...
        Assert.assertEquals(0, Converts.withMybatisPlusMapstruct().toPageBean(DEFAULT_PAGE, StudentDO.class, true).getRecords().size());
        // 转换失败（未匹配到转换方法），开启了静默转换，且设置了默认值，应打印异常，并将默认值包装为 PageResult<默认值> 并返回，注意观察日志是否正确
        Assert.assertEquals(CollectionsUtil.getFirst(Converts.withMybatisPlusMapstruct().toPageBean(DEFAULT_PAGE, StudentDO.class, DEFAULT_STUDENT_DO, true).getRecords()), DEFAULT_STUDENT_DO);

        // ================ 测试转换 null ================
        Assert.assertEquals(0, Converts.withMybatisPlusMapstruct().toPageBean(null, null).getRecords().size());
        Assert.assertEquals(0, Converts.withMybatisPlusMapstruct().toPageBean(null, null, null, null).getRecords().size());
    }
