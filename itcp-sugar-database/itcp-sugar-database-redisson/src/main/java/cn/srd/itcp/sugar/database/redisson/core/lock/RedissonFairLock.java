...
import cn.srd.itcp.sugar.database.redisson.exception.RedissonGenerateLockNameFailedException;
import cn.srd.itcp.sugar.database.redisson.support.lock.RedissonFairLockAspect;
...
/**
 * 基于 Redisson 的分布式单点公平锁，直接标记在方法上，无需侵入代码即可完成加锁释放锁操作；
 * <pre>
 *   一、关于公平与非公平锁的介绍与使用建议：
 ...
 *          显式指定 fieldName = "id", fieldOrder = "2"，表示使用方法形参列表的第二个参数中的 id 字段值作为锁名，即 book 中 id 的值作为锁名；若 id 的值为空，抛出 {@link RedissonGenerateLockNameFailedException} 异常；
 ...
 *          当 waitTime 的值为 0 时（默认），使用 {@link RedissonLockTemplate#lock(Supplier, String, long, TimeUnit)} 的方式上锁；
 *          当 waitTime 的值大于 0 时，使用 {@link RedissonLockTemplate#tryLock(Supplier, String, long, long, TimeUnit)} 的方式上锁；
...
@Documented
public @interface RedissonFairLock {
