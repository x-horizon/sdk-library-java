...
import cn.library.java.orm.mybatis.contract.postgresql.handler.JdbcJsonbListObjectTypeHandler;
 ...
 */
public class JavaListObjectMappingJdbcJsonbTypeHandler<T extends NullableObject> extends JdbcJsonbListObjectTypeHandler<T> {
    ...
    @Override
    public Object convertToJdbcComplexObject(List<T> javaObject) {
        return convertObjectToJsonb(javaObject.stream().filter(NullableObject::isNotNull).toList());
    }

    @SneakyThrows
    @Override
    public List<T> convertToJavaObject(ResultSet resultSet, String columnName) {
        return convertJsonbStringToList(resultSet.getString(columnName), getJavaType(columnName));
    }
    ...
    private synchronized void initDatabaseColumnNameMappingJavaClassMap() {
        databaseColumnNameMappingJavaClassMap = Annotations.getAnnotationValueMappingAnnotatedClassMap(Table.class, String.class)
                ...
                .filter(field -> Comparators.equalsAny(AnnotationUtil.getAnnotationValue(field, Column.class, "typeHandler"), JavaListObjectMappingJdbcJsonbTypeHandler.class))
                .map(field -> Collections.ofPair(Annotations.getAnnotationValue(field, Column.class, String.class), (Class<T>) Types.getEmbedGenericTypeClass(field.getDeclaringClass(), field.getName())))
