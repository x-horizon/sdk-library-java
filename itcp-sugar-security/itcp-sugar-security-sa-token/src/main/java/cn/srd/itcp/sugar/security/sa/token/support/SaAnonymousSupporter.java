...
import cn.srd.itcp.sugar.security.sa.token.core.SaAnonymous;
 ...
 */
public class SaAnonymousSupporter implements InitializingBean {

    /**
     * 收集标记了 {@link SaAnonymous} 的类或方法对应的所有接口，这些接口无需鉴权，用于匿名访问；
     */
    @Getter
    ...
    public void afterPropertiesSet() {
        SpringsUtil.getBean(RequestMappingHandlerMapping.class).getHandlerMethods().forEach((requestMappingInfo, requestMappingMethod) -> {
            ...
            Optional.ofNullable(AnnotationUtils.findAnnotation(requestMappingMethod.getMethod(), SaAnonymous.class)).ifPresent(anonymous -> {
            ...
            Optional.ofNullable(AnnotationUtils.findAnnotation(requestMappingMethod.getBeanType(), SaAnonymous.class)).ifPresent(anonymous -> {
                Set<String> patterns = requestMappingInfo.getPatternsCondition().getPatterns();
                patterns.forEach(url -> saAnonymousUrlsToExcludes.add(ReUtil.replaceAll(url, URI_PATH_VARIABLE_REPLACE_TO_ASTERISK_PATTERN, StringPool.ASTERISK)));
            });
        });
