...
public class MapstructConvertsTest {
    ...
    public void testMapstructConverts() {
        ...
        Assert.assertEquals(Collections.getFirst(Converts.withGenericMapstruct().toBeans(null, StudentVO.class, DEFAULT_STUDENT_VO)), DEFAULT_STUDENT_VO);
        // 转换带默认值，源对象为 empty 的情况下，将默认值包装为 ArrayList<默认值> 并返回
        Assert.assertEquals(Collections.getFirst(Converts.withGenericMapstruct().toBeans(new ArrayList<>(), StudentVO.class, DEFAULT_STUDENT_VO)), DEFAULT_STUDENT_VO);
        // 转换失败（未匹配到转换方法），未开启静默转换，应抛出异常
        ...
        Assert.assertEquals(Collections.getFirst(Converts.withGenericMapstruct().toBeans(StudentVO.newVOs(), StudentVO.class, DEFAULT_STUDENT_VO, true)), DEFAULT_STUDENT_VO);
