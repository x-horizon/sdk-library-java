...
public abstract class ContextAwareActor<T> extends DefaultActor<T> {
    ...
    public boolean process(ActorEvent<T> event) {
        if (log.isTraceEnabled()) {
            log.trace("{}}[{}] is processing event: {}", ModuleView.CONCURRENT_ACTOR_SYSTEM, getActorId(), event);
        }
        boolean success = doProcess(event);
        if (!success) {
            log.warn("{}[{}] process event failed: {}", ModuleView.CONCURRENT_ACTOR_SYSTEM, getActorId(), event);
        }
        return success;
    }
    ...
    public ProcessFailureStrategy onProcessFailure(Throwable throwable) {
        log.error("{}[{}] process event exception: ", ModuleView.CONCURRENT_ACTOR_SYSTEM, getActorId(), throwable);
