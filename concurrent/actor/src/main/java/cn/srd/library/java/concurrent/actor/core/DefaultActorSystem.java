...
import cn.srd.library.java.concurrent.actor.exception.ActorNotRegisteredException;
...
public class DefaultActorSystem implements ActorSystem {
    ...
    private ActorMailbox createActor(String dispatcherId, ActorCreator creator, ActorId parentId) {
        ...
        if (dispatcher == null) {
            log.warn("{}Dispatcher with id [{}] is not registered!", ModuleView.CONCURRENT_ACTOR_SYSTEM, dispatcherId);
            throw new RuntimeException("Dispatcher with id [" + dispatcherId + "] is not registered!");
        }
        ...
        if (actorMailbox != null) {
            log.warn("{}Actor with id [{}] is already registered!", ModuleView.CONCURRENT_ACTOR_SYSTEM, actorId);
        } else {
            ...
            try {
                ...
                if (actorMailbox == null) {
                    log.debug("{}Creating actor with id [{}]!", ModuleView.CONCURRENT_ACTOR_SYSTEM, actorId);
                    ...
                    log.debug("{}Actor with id [{}] is already registered!", ModuleView.CONCURRENT_ACTOR_SYSTEM, actorId);
                }
            } finally {
    ...
    public void stop() {
        dispatchers.values().forEach(dispatcher -> {
            ...
            try {
                ...
                log.warn("{}[{}] Failed to stop dispatcher", ModuleView.CONCURRENT_ACTOR_SYSTEM, dispatcher.getDispatcherId(), e);
            }
        });
        if (scheduler != null) {
