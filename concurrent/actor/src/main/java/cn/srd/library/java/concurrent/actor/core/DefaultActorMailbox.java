...
public final class DefaultActorMailbox implements ActorMailbox {
    ...
    private void tryInit(int attempt) {
        try {
            log.debug("{}[{}] Trying to init actor, attempt: {}", ModuleView.CONCURRENT_ACTOR_SYSTEM, selfId, attempt);
            ...
            log.error("{}[{}] Failed to init actor, attempt: {}", ModuleView.CONCURRENT_ACTOR_SYSTEM, selfId, attempt, throwable);
            ...
            if (strategy.isStop() || (systemSettings.getMaxActorInitAttempts() > 0 && attemptIdx > systemSettings.getMaxActorInitAttempts())) {
                log.error("{}[{}] Failed to init actor, attempt {}, going to stop attempts.", ModuleView.CONCURRENT_ACTOR_SYSTEM, selfId, attempt, throwable);
                stopReason = ActorStopReason.INIT_FAILED;
                destroy();
            } else if (strategy.getRetryDelay() > 0) {
                log.error("{}[{}] Failed to init actor, attempt {}, going to retry in attempts in {}ms", ModuleView.CONCURRENT_ACTOR_SYSTEM, selfId, attempt, strategy.getRetryDelay());
                log.error("{}[{}] Error", ModuleView.CONCURRENT_ACTOR_SYSTEM, selfId, throwable);
                ...
                log.error("{}[{}] Failed to init actor, attempt {}, going to retry immediately", ModuleView.CONCURRENT_ACTOR_SYSTEM, selfId, attempt);
                log.error("{}[{}] Error", ModuleView.CONCURRENT_ACTOR_SYSTEM, selfId, throwable);
        ...
        if (ready.get() == READY) {
            if (hasNewEvent || !highPriorityActorEventQueue.isEmpty() || !normalPriorityActorEventQueue.isEmpty()) {
                if (busy.compareAndSet(FREE, BUSY)) {
                    ...
                    log.trace("{}process queue async - [{}] is busy, has new event or not: {}", ModuleView.CONCURRENT_ACTOR_SYSTEM, selfId, hasNewEvent);
                }
            } else {
                log.trace("{}process queue async - [{}] is empty, has new event or not: {}", ModuleView.CONCURRENT_ACTOR_SYSTEM, selfId, hasNewEvent);
            }
        } else {
            log.trace("{}process queue async - [{}] is not ready, has new event or not: {}", ModuleView.CONCURRENT_ACTOR_SYSTEM, selfId, hasNewEvent);
        }
    }
        ...
        if (ready.get() == READY) {
            if (hasNewEvent || !highPriorityActorEventQueue.isEmpty() || !normalPriorityActorEventQueue.isEmpty()) {
                if (busy.compareAndSet(FREE, BUSY)) {
                    ...
                    log.trace("{}process queue sync - [{}] is busy, has new event or not: {}", ModuleView.CONCURRENT_ACTOR_SYSTEM, selfId, hasNewEvent);
                }
            } else {
                log.trace("{}process queue sync - [{}] is empty, has new event or not: {}", ModuleView.CONCURRENT_ACTOR_SYSTEM, selfId, hasNewEvent);
            }
        } else {
            log.trace("{}process queue sync - [{}] is not ready, has new event or not: {}", ModuleView.CONCURRENT_ACTOR_SYSTEM, selfId, hasNewEvent);
        }
    }
            ...
            if (event != null) {
                try {
                    if (log.isTraceEnabled()) {
                        log.trace("{}[{}] Going to process event: {}", ModuleView.CONCURRENT_ACTOR_SYSTEM, selfId, event);
                    }
                    actor.process(event);
                } catch (Throwable throwable) {
                    log.error("{}[{}] Failed to process event: {}", ModuleView.CONCURRENT_ACTOR_SYSTEM, selfId, event, throwable);
    ...
    public void destroy() {
        ...
        dispatcher.getExecutor().execute(() -> {
            try {
                ...
                log.error("{}[{}] Failed to destroy actor: {}", ModuleView.CONCURRENT_ACTOR_SYSTEM, selfId, throwable);
            }
        });
    }
