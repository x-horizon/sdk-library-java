package cn.srd.itcp.sugar.component.convert.all.mapstruct.benchmark;
...
public class StringBenchmarkTest {
     ...
     * 字符串拼接考虑三种方式：
     * 1、使用 + 拼接，编码不友好，性能最快；
     * 2、使用 {@link StringsUtil#format(CharSequence, Object...)} 拼接，编码友好，性能最慢，不适合此处；
    ...
    public void test() {
        ...
        for (int i = 0; i < DATA_SIZE; i++) {
            String b = CONCAT_STR_1 + StringPool.SLASH + CONCAT_STR_2;
        }

        // 开始
        Instant now1 = Instant.now();
        for (int i = 0; i < DATA_SIZE; i++) {
            String b = StringsUtil.format("{}/{}", CONCAT_STR_1, CONCAT_STR_2);
        }
        System.out.println("StringsUtil.format, " + DATA_SIZE + "条数据，耗时：" + ChronoUnit.MILLIS.between(now1, Instant.now()) + "ms");
